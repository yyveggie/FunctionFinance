...
from crawler.usable_ip import Usable_IP
import os
...
import signal
import re
from crawler.filing.secSpecificCompanySpecificAnnouncement.utils import get_filing_urls_to_download
from crawler.filing.secSpecificCompanySpecificAnnouncement.prepline_sec_filings.sections import (
    ALL_SECTIONS,
    SECTIONS_10K,
    SECTIONS_10Q,
    SECTIONS_S1,
...
from crawler.filing.secSpecificCompanySpecificAnnouncement.prepline_sec_filings.sec_document import (
    REPORT_TYPES,
    VALID_FILING_TYPES,
    SECDocument,
)
...
try:
    ...
    def convert_to_isd(elements: List[Element]) -> List[Dict[str, Any]]:
        ...
        for element in elements:
            # 检查 element 是否有 to_dict 方法
            if hasattr(element, 'to_dict'):
                section = element.to_dict()
                isd.append(section)
            else:
                # 如果 element 没有 to_dict 方法，可以选择跳过或记录
...
class SECExtractor(Usable_IP):
    ...
    def get_text_from_url(self, url: str):
        ...
        all_narratives, filing_type = self.pipeline_api(
            text, m_section=self.sections)
        all_narrative_dict = dict.fromkeys(all_narratives.keys())

        for section in all_narratives:
            all_narrative_dict[section] = self.get_all_text(
                section, all_narratives)
    ...
    def pipeline_api(self, text, m_section=[], m_section_regex=[]):
        ...
        for i, section_regex in enumerate(m_section_regex):
            ...
            with timeout(seconds=5):
                section_elements = sec_document.get_section_narrative(
                    regex_num)
    ...
    def get_filing(self, url: str, company: str, email: str) -> str:
        ...
        response = self._request_session('get', url, session=session)
        response.raise_for_status()
        return response.text

    def _get_session(self, company: Optional[str] = None, email: Optional[str] = None, proxies=None) -> requests.Session:
