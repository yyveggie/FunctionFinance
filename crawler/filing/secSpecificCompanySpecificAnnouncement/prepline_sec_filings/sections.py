...
class SECSection(Enum):
    ...
    MANAGEMENT_DISCUSSION = re.compile(
        r"^management(?:[\u2019']s)? discussion")
    BUSINESS = re.compile(r"^business$")
    MANAGEMENT = re.compile(
        r"^(?:(?:our )?management)|(?:executive officers)$")
    COMPENSATION = re.compile(r"compensation")
    RELATED_PARTY_TRANSACTIONS = re.compile(
        r"(?:relationships|related).*transactions")
...
def validate_section_names(section_names: List[str]):
    ...
    if len(section_names) == 1 and section_names[0] == ALL_SECTIONS:
        ...
        raise ValueError(
            f"{ALL_SECTIONS} may not be specified with other sections")
    ...
    if invalid_names:
        raise ValueError(
            f"The following section names are not valid: {invalid_names}")
    return None
