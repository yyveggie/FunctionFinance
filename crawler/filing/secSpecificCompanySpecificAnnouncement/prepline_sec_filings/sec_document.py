...
from crawler.filing.secSpecificCompanySpecificAnnouncement.prepline_sec_filings.sections import SECSection
...
class SECDocument(HTMLDocument):
    ...
    def get_table_of_contents(self) -> HTMLDocument:
        ...
        try:
            ...
            print(f"Error processing elements for sklearn format: {e}")
            # 返回一个空的 HTMLDocument 实例，或者其他合适的默认值
    ...
    def get_section_narrative(self, section: SECSection) -> List[NarrativeText]:
        ...
        section_start_element = get_element_by_title(
            reversed(
                doc_after_section_toc.elements), section_toc.text, self.filing_type
        )
...
def is_section_elem(
    ...
    if section is SECSection.RISK_FACTORS:
        ...
        def _is_matching_section_pattern(text):
            return bool(
                re.search(section.pattern, clean_sec_text(
                    text, lowercase=True))
            )
...
def to_sklearn_format(elements: List[Element]) -> npt.NDArray[np.float32]:
    ...
    is_title: npt.NDArray[np.bool_] = np.array(
        [is_possible_title(el.text)
         for el in elements][: len(elements)], dtype=bool
    )
    title_locs = np.arange(len(is_title)).astype(
        np.float32)[is_title].reshape(-1, 1)
    return title_locs
