...
def convert_json_to_dropdown_options(file_path):
    try:
        with open(file_path, 'r') as file:
            data = json.load(file)
        dropdown_options = [{'label': key, 'value': value}
...
class UserDetail(BaseModel):
    company_en: Optional[List[str]] = Field(
        description="Extract or infer the English name of the mentioned company from the query. REMEMBER ONLY ONE COMPANY.",
        default=[],
        examples=[["Apple"]]
    )
    companies_en: Optional[List[str]] = Field(
        description="Extract or infer the English name of the mentioned companies from the query. REMEMBER MORE THAN ONE COMPANY.",
        default=[],
        examples=[["Apple", "Google"]]
    )
    company_cn: Optional[List[str]] = Field(
        description="Extract or infer the Chinese name of the mentioned company from the query. REMEMBER ONLY ONE COMPANY.",
        default=[],
        examples=[["苹果"]]
    )
    companies_cn: Optional[List[str]] = Field(
        description="Extract or infer the Chinese name of the mentioned companies from the query. REMEMBER MORE THAN ONE COMPANY.",
        default=[],
        examples=[["苹果", "谷歌"]]
    )
    listing_exchange: Optional[List[str]] = Field(
        description="All stock exchanges on which the companies are listed, usually you need to infer from your knowledge. If there are multiple companies, you need to have separate lists for each.",
        default=[],
        examples=[["Nasdaq", "SSE"]]
    )
    ticker: Optional[List[str]] = Field(
        description="The stock ticker corresponding to the stock exchange on which the company is listed, usually you need to infer from your knowledge.",
        default=[],
        examples=[["AAPL", "GOOGL", "BABA", "00700"]]
    )
    ...
    timeframe: Optional[List[str]] = Field(
        ...
        default=[],
        examples=[["today", "week", "month",
                   "ytd", "year", "3 years", "5 years"]]
    )
    quarter: Optional[List[str]] = Field(
        description="Extract the relevant time range from the query, which is usually related to financial reporting, performance analysis, and forecasting and planning. It can only select one from these four elements: 'Q1', 'Q2', 'Q3', 'Q4'",
        default=[],
...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        id='spinner2',
        options=convert_json_to_dropdown_options(
            "ipos_upcoming_and_filings_categories.json"),
...
sub_options = {
    'category': [{'label': '年报', 'value': 'category_ndbg_jjgg;'}, {'label': '半年报', 'value': 'category_bndbg_szsh;'}, {'label': '一季报', 'value': 'category_yjdbg_szsh;'}, {'label': '三季报', 'value': 'category_sjdbg_szsh;'}, {'label': '业绩预告', 'value': 'category_yjygjxz_szsh;'}, {'label': '权益分派', 'value': 'category_qyfpxzcs_szsh;'}, {'label': '董事会', 'value': 'category_dshgg_szsh;'}, {'label': '监事会', 'value': 'category_jshgg_szsh;'}, {'label': '股东大会', 'value': 'category_gddh_szsh;'}, {'label': '日常经营', 'value': 'category_rcjy_szsh;'}, {'label': '公司治理', 'value': 'category_gszl_szsh;'}, {'label': '中介报告', 'value': 'category_zj_szsh;'}, {'label': '首发', 'value': 'category_sf_szsh;'}, {'label': '增发', 'value': 'category_zf_szsh;'}, {'label': '股权激励', 'value': 'category_gqjl_szsh;'}, {'label': '配股', 'value': 'category_pg_szsh;'}, {'label': '解禁', 'value': 'category_jj_szsh;'}, {'label': '公司债', 'value': 'category_gszq_szsh;'}, {'label': '可转债', 'value': 'category_kzzq_szsh;'}, {'label': '其他融资', 'value': 'category_qtrz_szsh;'}, {'label': '股权变动', 'value': 'category_gqbd_szsh;'}, {'label': '补充更正', 'value': 'category_bcgz_szsh;'}, {'label': '澄清致歉', 'value': 'category_cqdq_szsh;'},
                 {'label': '风险提示', 'value': 'category_fxts_szsh;'}, {'label': '特别处理和退市', 'value': 'category_tbclts_szsh;'}, {'label': '退市整理期', 'value': 'category_tszlq_szsh;'}, {'label': '临时公告', 'value': 'category_lsgg;'}, {'label': '定期公告', 'value': 'category_dqgg;'}, {'label': '中介机构公告', 'value': 'category_zjjg;'}, {'label': '持续信息披露', 'value': 'category_cxpl;'}, {'label': '首次信息披露', 'value': 'category_scpl;'}, {'label': '招募设立', 'value': 'category_jjzm_jjgg;'}, {'label': '说明书更新', 'value': 'category_smsgx_jjgg;'}, {'label': '中报', 'value': 'category_bndbg_jjgg;'}, {'label': '季报', 'value': 'category_jdbg_jjgg;'}, {
                     'label': '净值', 'value': 'category_jzgg_jjgg;'}, {'label': '投资组合', 'value': 'category_zhtz_jjgg;'}, {'label': '申购赎回', 'value': 'category_sgsh_jjgg;'}, {'label': '基金费率', 'value': 'category_jjfl_jjgg;'}, {'label': '销售渠道', 'value': 'category_xsqd_jjgg;'}, {'label': '分红', 'value': 'category_fh_jjgg;'}, {'label': '高管及基金经理', 'value': 'category_ggjjjl_jjgg;'}, {'label': '持有人大会', 'value': 'category_fecyr_jjgg;'}, {'label': '基本信息变更', 'value': 'category_jbxxbg_jjgg;'}, {'label': '其他', 'value': 'category_jgjg_qt;'}, {'label': '债券发行上市', 'value': 'category_zqfxss_zqgg;'},
...
def update_output(n_clicks, value1, subvalue1, value2, value3, value4, value5, value6, query):
    if n_clicks > 0:
        logging.info(
            f"选择1: {value1}, 子选择1: {subvalue1}, 选择2: {value2}, 选择3: {value3}, 选择4: {value4}, 选择5: {value5}, 选择6: {value6}, 查询: {query}")
        custom_print("Query 解析中...")
        result = process_query(query, value1, subvalue1,
...
def process_query(query, value1, subvalue1, value2, value3, value4, value5, value6):
    user: UserDetail = client.chat.completions.create(
        ...
        messages=[
            ...
            3. query:
                ...
                response:
                    company_en: ['NVIDIA'],
                    companies_en: ['NVIDIA', 'TECENT'],
                    company_cn: ['英伟达'],
                    companies_cn: ['英伟达', '腾讯'],
                    listing_exchange: ['Nasdaq', 'HKEX'],
                    ticker: ['NVDA', '0700'],
    ...
    data = {
        ...
        "companies_en": user.companies_en,
        "company_cn": user.company_cn,
        "companies_en": user.companies_cn,
        "listing_exchange": user.listing_exchange,
        "ticker": user.ticker,
