...
@tool("Stock_trend_chart_analysis")
def vision(ticker: str, indicators: List[str]):
    """
    分析指定股票趋势图，包括股价趋势和指定金融指标（如果有的话）趋势。
    
    参数:
    - ticker(str): 你感兴趣的股票代码，例如:'AAPL'
    - indicators(List[str]): 你感兴趣的金融指标趋势，只能从以下列表中选择，最多三项
        ['MACD', 'RSI', 'Stochastic Oscillator', 'Bollinger Bands', 'Moving Average', 'On Balance Volume', 'Average True Range', 'Ichimoku Clouds']
       例如: ['MACD', 'RSI', 'Stochastic Oscillator']
    """
    print('正在调用读图模式, 参数为:', ticker, indicators)
    resp = run(ticker, indicators)
    print('调用完成，以下是结果:')
    print(resp)
    return resp
